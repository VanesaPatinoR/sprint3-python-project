...
def filter_by_genre(data, genre='Platform'):
    ...
    for game in data:
        if game[GENRE] == genre: 
           result.append(game)
    return result
result1 = filter_by_genre(video_game_sales)
print("Platform games:", result1)

result2= filter_by_genre(video_game_sales,genre='Sports')
print("Sports games:",result2)

def get_summary(game):
        return  (F"{game[NAME]} ({game[YEAR]}) - {game[GENRE]} -      ${game[GLOBAL_SALES]}M")

result1=get_summary(video_game_sales[2])
print(result1)
